 ...
 * - Carga el texto de la barra de noticias, que vive en la columna izquierda
...
import { getTickerText } from './lib/data';
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  if (isSection || route === '/404') {
    const ticker = await getTickerText();
    context.locals.tickerText = ticker.data;
    if (isSection && context.cache.enabled) {
      if (ticker.ok) context.cache.set(PUBLIC_CACHE);
